...
describe('user-preview directive', () => {
    ...
    test('keeps popup open on popup mouseover and hides on popup mouseleave', () => {
        const binding = { value: user } as any;
        (UserPreviewDirective as any).mounted(el, binding, null);
        ...
        const popupArgs = popupSpy.mock.calls[0];
        const showingRef = popupArgs[1].showing;
        const events = popupArgs[2];

        // Simulate el mouseleave (triggers hide timer)
        el.dispatchEvent(new MouseEvent('mouseleave'));
        vi.advanceTimersByTime(250); // wait halfway

        // Simulate popup mouseover (cancels hide timer)
        events.mouseover();
        vi.advanceTimersByTime(500);
        assert.isTrue(showingRef.value); // still open

        // Simulate popup mouseleave (re-triggers hide timer)
        events.mouseleave();
        vi.advanceTimersByTime(250);
        assert.isTrue(showingRef.value);
        vi.advanceTimersByTime(250);
        assert.isFalse(showingRef.value); // closed
