...
function createServices() {
	const metaService = {} as unknown as jest.Mocked<MetaService>;
	const roleService = {
	...
	return { metaService, roleService, noteEntityService };
}
...
describe('HybridTimelineChannel', () => {
	describe('init', () => {
		test('returns early when ltlAvailable policy is false', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: false } as never);
			const connection = createConnection();
			const service = new HybridTimelineChannelService(metaService, roleService, noteEntityService);
		...
		test('subscribes to notesStream when ltlAvailable policy is true', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: true } as never);
			const connection = createConnection();
			const service = new HybridTimelineChannelService(metaService, roleService, noteEntityService);
			const channel = service.create('channel1', connection);
	...
	describe('onNote', () => {
		...
		test('sends local public note even from non-followed user', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: true } as never);
			const connection = createConnection();
			const service = new HybridTimelineChannelService(metaService, roleService, noteEntityService);
			const channel = service.create('channel1', connection);
			await channel.init({ withReplies: false });
			const note = createPackedNote({ userId: 'other1', text: 'local public note' });
		...
		test('filters remote non-followed note', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: true } as never);
			const connection = createConnection();
			const service = new HybridTimelineChannelService(metaService, roleService, noteEntityService);
		...
		test('repacks followers/specified note and filters when hidden', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: true } as never);
		...
		test('repacks followers/specified note and sends when not hidden', async () => {
			const { metaService, roleService, noteEntityService } = createServices();
			roleService.getUserPolicies.mockResolvedValue({ ltlAvailable: true } as never);
		...
		test('repacks renote when renoteId is present', async () => {
			...
			const channel = service.create('channel1', connection);
			await channel.init({ withReplies: true });
			const note = createPackedNote({ renoteId: 'renote1' });
