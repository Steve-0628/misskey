...
function createRoleService(policies: { ltlAvailable: boolean } = { ltlAvailable: true }): jest.Mocked<RoleService> {
	return {
...
describe('LocalTimelineChannel', () => {
	describe('init', () => {
		test('subscribes to notesStream when ltlAvailable policy is true', async () => {
			const metaService = createMetaService();
			const roleService = createRoleService({ ltlAvailable: true });
			const noteEntityService = createNoteEntityService();
			const connection = createConnection();
			const service = new LocalTimelineChannelService(metaService, roleService, noteEntityService);
		...
		test('does not subscribe when ltlAvailable policy is false', async () => {
			const metaService = createMetaService();
			const roleService = createRoleService({ ltlAvailable: false });
			const noteEntityService = createNoteEntityService();
			const connection = createConnection();
			const service = new LocalTimelineChannelService(metaService, roleService, noteEntityService);
			const channel = service.create('channel1', connection);
	...
	describe('onNote', () => {
		test('sends local public note', async () => {
			const metaService = createMetaService();
			const roleService = createRoleService({ ltlAvailable: true });
			const noteEntityService = createNoteEntityService();
			const connection = createConnection();
		...
		test('filters remote note', async () => {
			const metaService = createMetaService();
			const roleService = createRoleService({ ltlAvailable: true });
			const noteEntityService = createNoteEntityService();
			const connection = createConnection();
