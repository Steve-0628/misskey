...
describe('ImportBlockingProcessorService', () => {
	...
	let queueService: {
		createImportBlockingToDbJob: jest.MockedFunction<QueueService['createImportBlockingToDbJob']>;
		createBlockJob: jest.MockedFunction<QueueService['createBlockJob']>;
	};
	...
	beforeEach(async () => {
		...
		queueService = {
			createImportBlockingToDbJob: jest.fn(),
	...
	test('does nothing when user not found', async () => {
		...
		expect(queueService.createImportBlockingToDbJob).not.toHaveBeenCalled();
	...
	test('creates import-to-db jobs from csv', async () => {
		const user = { id: 'user1' } as User;
		const file = { id: 'file1', url: 'https://example.com/blocking.csv' } as DriveFile;
		usersRepository.findOneBy.mockResolvedValue(user);
		...
		expect(downloadService.downloadTextFile).toHaveBeenCalledWith('https://example.com/blocking.csv');
		expect(queueService.createImportBlockingToDbJob).toHaveBeenCalledWith(
			{ id: 'user1' },
	...
	test('processDb resolves remote user and creates block job', async () => {
		...
		expect(queueService.createBlockJob).toHaveBeenCalledWith([
	...
	test('processDb skips self', async () => {
		...
		expect(queueService.createBlockJob).not.toHaveBeenCalled();
	});
